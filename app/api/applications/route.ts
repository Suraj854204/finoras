...
export async function POST(req: Request) {
  try {
    ...
    if (!parsed.success)
      return withCors(
        error("Invalid application details", 400, parsed.error.flatten()),
      );
    ...
    const result = await db.$transaction(async (tx) => {
      ...
      const plan = await tx.eMIPlan.findFirst({
        where: {
          id: input.emiPlanId,
    ...
    if (message === "OUT_OF_STOCK")
      return withCors(error("This variant is out of stock", 409));
    if (message === "VARIANT_NOT_FOUND")
      return withCors(error("Invalid product variant", 400));
    if (message === "PLAN_NOT_FOUND")
      return withCors(error("Invalid EMI plan", 400));
